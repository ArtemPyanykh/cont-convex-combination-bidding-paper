...
\newcommandx*\G[2][1={p_{1,1}}, 2=\tau_1]{\ensuremath{G_{n+1} \left( #1, #2 \right)}}
...
\begin{proof}
  ...
  \end{equation*}
  Пусть теперь $p_{2,1} = f(\alpha), \; \alpha \in [0, 1]$.
  Введем обозначения
  \[
  ...
  \begin{equation}\label{eq:fp:lower:alpha-0-1}
    \begin{aligned}
      &\F{p_{2,1}} =
...
Если случайная величина $u$ равномерно распределена на $[0, 1]$, положим $p_{2,1} = h(u)$.
...
Заметим, что для $g_1^L(p_{1,1}, \tau_1)$ выполнено
\begin{equation*}
  g_1^L(p_{1,1},\tau_1) =&
  \int_I \biggl[
    \Ind_{p_{1,1}>p_{2,1}} (-\Co p_{1,1} - \Dco p_{2,1}) +
    \Ind_{p_{1,1}<p_{2,1}} (\Dco p_{1,1} + \Co p_{2,1})
  \biggr] \tau_1(\di p_{2,1}).
\end{equation*}
Аналогично для $g_1^H(p_{1,1}, \tau_1)$ верно равенство
\begin{multline*}
  g_1^H(p_{1,1},\tau_1) =
  \int_I \biggl[
    \Ind_{p_{1,1}>p_{2,1}} (1 -\Co p_{1,1} - \Dco p_{2,1}) + \\
    + \Ind_{p_{1,1}<p_{2,1}} (\Dco p_{1,1} + \Co p_{2,1} - 1)
  \biggr] \tau_1(\di p_{2,1}).
\end{multline*}
...
\begin{multline*}%\label{eq:W:lowerBound:parameterized}
  \G[p_{1,1}][h] = W_{2,n}\left(
    x - \int_0^1\left[\Ind_{h(u) < p_{1,1}} - \Ind_{h(u) > p(u)}\right] \di u
  \right) \; - \\
  - \int_0^1 \left[
...
\begin{lemma}\label{lemma:W:lowerBound:parametrized}
  Для любого $x \in \mathbb{R}$ выполнено
  \begin{equation*}
    W_{2,n+1}(x) \geqslant \sup_h \inf_{p_{1,1}} G(p_{1,1}, h),
  \end{equation*}
  где $\G[p_{1,1}][h]$ определяется следующим образом:
\end{lemma}
