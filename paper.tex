...
В [\ref{lit:demeyer02}] рассматривается параметризация одношаговой стратегии инсайдера функциями $f$ и $Q$, которые используются для генерации маргинального распределения $p_1$ и апостериорной вероятности $P(p_1)$ соответственно.
...
В [\ref{lit:demeyer02}] показано, что функции $f$ и $Q$ должны удовлетворять следующим свойствам, чтобы быть параметризацией некоторой одношаговой стратегии $\sigma_1$ первого игрока в игре $G_n(P)$:
\vspace{-2.0em}
\begin{adjustwidth}{\parindent}{0em}
  \begin{subequations}
    ...
    \begin{flalign}
      ...
      \bullet \quad \forall u_1, u_2 \in [0, 1]: f(u_1) = f(u_1) \implies Q(u_1) = Q(u_2). &&
    \end{flalign}
  \end{subequations}
\end{adjustwidth}
...
\end{equation*}
Тогда аналогично [\ref{lit:demeyer02}] можно показать, что при подстановке \eqref{eq:qFormula} в \eqref{eq:phi(q)} выполняется равенство
\begin{equation}\label{eq:phi=k*}
  \Phi(Q) = K^*(P).
\end{equation}

Отметим, что хотя функция $f(u)$ зависит от коэффициента $\Co$, функции $Q(u)$ и $\Phi(Q)$ от него не зависят и по форме совпадают с аналогичными выражениями в [\ref{lit:demeyer02}].
Данный факт позволит нам без изменений использовать соотношения из [\ref{lit:demeyer02}] между нижним и верхним значениями прямой и двойственной игр.
...
\begin{proof}
  ...
  где $X$ -- случайная величина распределенная на отрезке $[\Dco, u]$ с плотностью
...
\begin{proof}
  ...
  \begin{multline*}
    \F{p_2} = \int_0^1 \left[
      Q(u) - \Co f(u) - \Dco p_2
      \right] \di u \; + \\
    + \int_0^1 V_{1,n}(Q(u)) \di u \geqslant \F{f(0)} = \Phi(Q) = K^*(P).
  \end{multline*}
  Аналогично можно показать, что при $p_2 > f(1)$
  \begin{equation*}
    \F{p_2} \geqslant \F{f(1)} = \Phi(Q) = K^*(P).
  \end{equation*}
