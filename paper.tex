...
\engtitle{On a modification of the multistage bidding model with continuous bids and asymmetric information}
...
\engabstract{%
  This paper is concerned with a modification of the multistage bidding model with continuous bids.
...
В вышеупомянутых работах использовался одинаковый механизм проведения сделки, при котором акция продается по наибольшей из предложенных цен.
...
Фактически, в работах [\ref{lit:demeyer02},
\ref{lit:domansky07}--\ref{lit:sandomirskaya12}] коэффициент $\Co$ равен $1$.
Обобщение модели c дискретными ставками на случай произвольного $\Co$ было
рассмотрено в [\ref{lit:pyanykh16}]. В данной работе обобщение на случай
произвольного $\Co$ проведено для модели игры с непрерывными ставками.

Отметим, что в [\ref{lit:demeyer10}] рассмотрена непрерывная модель торгов с
достаточно общим механизмом проведения сделки, для которой автором найдена
асимптотика последовательности цен сделок. Однако, данный результат касается
бесконечных игр, в то время как в данной работе рассматривается игра конечной
продолжительности. Кроме того, одним из условий, накладываемых на механизм
проведения сделки, в рамках которых автором получены результаты, является
существование значения $n$-шаговой игры при любом априорном распределении цены
акции с конечным математическим ожиданием.
...
Поскольку значения прямой и двойственной игр не зависят от $\beta$ и совпадают с
аналогичными значениями в [\ref{lit:demeyer02}], и справедливы
неравенства %
$ Q(0) \leqslant Q(u) \leqslant Q(1),\ Q(0) \leqslant f(u) \leqslant Q(1),\ u
\in [0, 1], $ %
содержание следствия 4, теоремы 12 и леммы 9 из [\ref{lit:demeyer02}] об
...
\begin{example}
  ...
  \begin{figure}[h!]
    \begin{center}
      \includegraphics[height=6cm]{w2.eps}
      \fcaption{Функция $W_2'(x)$}
      \label{fig:w2}
    \end{center}
  \end{figure}
...
\item\label{lit:demeyer02}%
  De Meyer B., Saley H. \textit{On the strategic origin of Brownian motion in
    finance} // International Journal of Game Theory. 2002. V. 31. P. 285--319.

\item\label{lit:aumann}%
  Aumann R.J., Maschler M.B. \textit{Repeated Games with Incomplete
    Information}. The MIT Press, Cambridge, London.

\item\label{lit:domansky07}%
  Domansky V. \textit{Repeated games with asymmetric information and random
    price fluctuations at finance markets} // International Journal of Game
  Theory. 2007. V. 36(2). P. 241--257.

\item\label{lit:kreps09}%
  Крепс В.Л. \textit{Повторяющиеся игры, моделирующие биржевые торги, и
    возвратные последовательности} // Известия РАН. Теория и системы управления.
  2009. № 4. С. 109--120.

\item\label{lit:sandomirskaya12}%
  Сандомирская М.С., Доманский В.К. \textit{Решение одношаговой игры биржевых
    торгов с неполной информацией} // Математическая теория игр и ее приложения.
  2012. 4. №1. С. 32-54.

\item\label{lit:chatterjee83}%
  Chatterjee K., Samuelson W. \textit{Bargaining under Incomplete Information}
  // Operations Research. 1983. V. 31. N. 5. P. 835--851.

\item\label{lit:pyanykh14}%
  Пьяных А.И. \textit{Об одной модификации модели биржевых торгов с инсайдером}
...
\item\label{lit:demeyer10}%
  De Meyer B. \textit{Price dynamics on a stock market with asymmetric
    information} // Games and Economic Behavior. 2010. V. 69. P. 42--71.
