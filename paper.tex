...
\begin{lemma}
  $V_{1,n}(\cdot)$ и $V_{2,n}(\cdot)$ -- вогнутые непрерывные функции. Кроме того\normalfont{,}
...
$\tilde{\sigma}$ -- семейство стратегий в игре продолжительности $n-1$, зависящих от
...
Следуя [\ref{lit:demeyer02}], рассмотрим двойственную игру $G^*_n(x)$,
определенную следующим образом. Перед началом игры инсайдер выбирает текущее
состояние $s \in S$. Если $s = H$, то он вынужден заплатить второму игроку
пенальти размера $x$ в конце игры. В остальном игра аналогична $G_n$.

Таким образом, стратегией первого игрока является пара $\dualFPS$, где
$P \in [0, 1]$, а $\sigma$ -- стратегия в $G_n$. Множество стратегий второго
игрока не отличается от аналогичного в $G_n$.
...
Приведем без доказательства следующие результаты (см. [\ref{lit:demeyer02}]).
\begin{lemma}
  \label{lemma:W:subdiff}
  $W_{2,n}(\cdot)$ -- вогнутая функция. Кроме того, для любого
  $x \in \mathbb{R}$ выполнено $\partial W_{2,n}(x) \in [0, 1]$. В частности,
  $W_{2,n}(\cdot)$ является липшицевой.
\end{lemma}
\begin{lemma}
  \label{lemma:W:lowerBound:general}
  Для любого $x \in \mathbb{R}$ выполняется неравенство
  \begin{equation*}
    W_{2,n+1} \geqslant \sup_{\tau_1} \inf_{p_{1,1}}
      W_{2,n}(x - g^H_1(p_{1,1}, \tau_1 + g^L(p_{1,1}, \tau_1)) - g^L_1(p_{1,1}, \tau_1),
  \end{equation*}
  где $g^H_1(p_{1,1},\tau_1) = g_1(1, p_{1,1}, \tau_1)$ и
