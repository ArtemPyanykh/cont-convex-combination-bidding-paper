...
Пусть $f(x)$ -- вогнутая функция, определенная на прямой, причем $f$ может принимать значение $-\infty$.
...
\end{equation*}
%
Справедливо утверждение [\ref{lit:rockafellar}, Теорема 23.5]):
\begin{equation}\label{eq:subdifferential:props}
  x^* \in \partial f(x) \iff x \in \partial f^*(x^*).
\end{equation}
Кроме того, имеют место следующие включения [\ref{lit:rockafellar}, \S 24]:
\begin{equation}\label{eq:dualrange}
  \mathrm{int} \; (\dom f) \subset \range \partial f^* \subset \dom f.
\end{equation}

В [\ref{lit:demeyer02}] показано, что $V_{1,n}(\cdot)$ и $W_{2,n}(\cdot)$ являются вогнутыми на своей области определения.
