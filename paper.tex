...
\end{center}
\begin{center}
  \large \sc Артем И. Пьяных$^*$\footnotetext{$^*$ Исследование выполнено при финансовой поддержке РФФИ в рамках научного проекта № 16-01-00353 а}\\ %первый автор
  \rm
...
На каждом шаге торгов игроки делают произвольные ставки из единичного отрезка.
...
Похожий результат имеет место и для нижнего значения игры (см. [\ref{lit:demeyer02}]).
\begin{lemma}
...
Аналогично предыдущему пункту, можно провести рассмотрение рекурсивной структуры игры $G^*_n(x)$ и получить следующий результат из [\ref{lit:demeyer02}].
...
В [\ref{lit:demeyer02}] рассматривается параметризация одношаговой стратегии инсайдера функциями $f$ и $Q$, которые используются для генерации маргинального распределения ставки $p_1$ и апостериорной вероятности $P(p_1)$ соответственно.
...
Отметим, что хотя функция $f(u)$ зависит от коэффициента $\Co$, выражения для функций $Q(u)$ и $\Phi(Q)$ от него не зависят и по форме совпадают с аналогичными выражениями в [\ref{lit:demeyer02}].
...
Отметим, что несмотря на зависимость $h(u)$ от $\beta$, выражение для функции $\Psi(h)$ от него не зависит и по форме совпадает с аналогичным выражением в [\ref{lit:demeyer02}].
...
\begin{proof}
  ...
  Аналогично можно показать, что при $p_1 > h(1)$ выполняется неравенство $\G[p_1][h] \geqslant \Psi(h)$.
  ...
  \]
  Так как $h(u)$ постоянная на $[\alpha^-, \alpha^+]$ и $h(\alpha) = p_1$ имеем
  \begin{multline*}
  ...
  \end{multline*}
  Используя выражение для $W_{2,n}(x + 1 - \alpha^- - \alpha^+)$ в \eqref{eq:g:+-}, получаем
  \begin{multline*}
    \G[p_1][h] \geqslant W_{2,n}(x + 1 - 2\alpha)
    - \int_\alpha^1 \biggl( \Dco h(\alpha) + \Co h(u) \biggr) \di u \; + \\
    + \int_0^\alpha \biggl( \Co h(\alpha) + \Dco h(u) \biggr) \di u = \G[h(\alpha)][h] = \Psi(h).
  \end{multline*}
  Таким образом, неравенство имеет место при любом $p_1 \in [0, 1]$.
\end{proof}

\section{Значение игры $\mathbf{G_n\left(P\right)}$}\label{sec:-game-value}
Поскольку выражения для нижних оценок в теоремах \ref{theorem:V:bound} и \ref{theorem:W:bound} совпадают с аналогичными выражениями в [\ref{lit:demeyer02}], справедливы все двойственные соотношения между $V_{1,n}(P)$ и $W_{2,n}(x)$, а также утверждения относительно оптимальности стратегий.
...
В заключение обоснуем сделанные ранее предположения о непрерывной дифференцируемости функций $V_{1,n}(\cdot)$, $V_{1,n}^*(\cdot)$ и $W_{2,n}(\cdot)$.
...
\end{proof}
%
Из \eqref{eq:f}, \eqref{eq:qFormula}, \eqref{eq:hu} и \eqref{eq:duality:relationships} получаем следующий алгоритм поиска оптимальных действий на первом шаге $n$-шаговой игры $G_n(P)$.
  \begin{enumerate}
  \item
    Найдем $\lambda$ как решение уравнения $P = W'_n(\lambda)$.
    $W'_n(\cdot)$ является кусочно-полиномиальной функцией $n$-го порядка, следовательно её корни могут быть найдены численно с любой заданной точностью.
  \item
    Найдем $Q(u) = W'_{n-1}(\lambda + 1 - 2u)$.
    В силу сказанного выше, $Q(u)$ является кусочно полиномиальной функцией порядка $n-1$.
  \item
    Найдем $f(u) = (u - \Dco)^{-2} \int_\Dco^u 2 (x - \Dco) Q(x) \di x$.
    Так как для расчета оптимальной ставки необходимо знать значение $f$ лишь в одной точке, оно может быть эффективно найдено численно.
  \item
    Так как выражения для $h$ и $f$ совпадают, положим $h = f$.
  \end{enumerate}

\begin{example}
  Найдем оптимальные стратегии игроков в одношаговой игре $G_1(P)$.
...
\begin{example}
  Найдем оптимальные стратегии игроков в игре $G_2(P)$.
  Для $W'_2(\cdot)$ справедливо представление
  \begin{equation*}
    W'_2(x) = \begin{cases}
      1, &\; x < -2,\\
  ...
  Тогда искомое $\lambda$ найдем как решение уравнения $(4 - 4x - x^2)/8 = P$, т.е. $\lambda = -2 + 2\sqrt{2 - 2P}$.
  Отсюда получим
  \begin{equation*}
  ...
  Таким образом, на первом шаге игры инсайдер получает $u_1$ как реализацию случайной величины $U_1$, распределенной на $[0, 1]$ с плотностью $Q(u)/P$, и делает ставку $f(u_1)$.
  %
  Второй игрок получает $u_2$ как реализацию случайной величины $U_2$, распределенной равномерно на $[0, 1]$, и делает ставку $h(u_2)$.
  %
  После этого игроки переходят к игре $G_1(P')$, где $P' = Q(u_1)$, решение которой было разобрано в предыдущем примере.
\end{example}
...
\item\label{lit:demeyer02}
  De Meyer B., Saley H. \textit{On the strategic origin of Brownian motion in finance} // International Journal of Game Theory. 2002. V. 31. P. 285--319.
...
\item\label{lit:pyanykh16}
  Пьяных А.И. \textit{Многошаговая модель биржевых торгов с асимметричной информацией и элементами переговоров} // Вестн. Моск. ун-та. Сер.15. Вычисл. матем. и киберн. 2016. №1. С. 34—40.
